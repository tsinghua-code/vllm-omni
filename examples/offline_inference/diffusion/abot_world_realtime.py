...
"""Run ABot-World as an in-process realtime AR-Diffusion session.

This example exercises the internal session API. Each JSONL event advances the
world by one three-latent-frame AR block and writes the latent plus metadata.
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run in-process realtime ABot-World session.")
    parser.add_argument("--model", default=_MODEL, help="HF model ID or local checkpoint path.")
...
def _load_events(path: Path) -> list[dict[str, Any]]:
    ...
    for line_number, raw_line in enumerate(path.read_text().splitlines(), start=1):
        ...
        if frames is not None:
            ...
            for frame in frames:
                if not isinstance(frame, list) or any(not isinstance(a, str) for a in frame):
    ...
    try:
        for chunk_index, event in enumerate(events):
            ...
            if len(output.images) != 1 or not isinstance(output.images[0], torch.Tensor):
                raise RuntimeError("Expected one latent tensor from each realtime ABot chunk.")
